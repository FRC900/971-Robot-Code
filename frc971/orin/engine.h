...
private:
    virtual void blobFromGpuImageWrappers(const std::vector<GpuImage<uint8_t>>& batchInput) = 0;
...
public:
    ...
    // Run inference.
    // Input format [input][batch][GpuImage]
    bool runInference(const std::vector<std::vector<GpuImage<uint8_t>>>& inputs, bool &buffersResized, const int32_t forceBatchSize = -1);
...
private:
    ...
    virtual nvinfer1::Dims inputDimsFromInputImage(const GpuImage<uint8_t>& batchInput, const nvinfer1::Dims &modelInputDims) = 0;
    virtual void blobFromGpuImageWrappers(const std::vector<GpuImage<uint8_t>>& batchInput, size_t bufferIdx) = 0;
