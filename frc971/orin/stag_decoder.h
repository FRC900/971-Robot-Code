...
// A GPU worker initialized for a specific model batch size
// Having different engines configures for static batch sizes allows
// us to use cudaGraphs for each batch size which is faster than
// dynamically changing the bacth size of a single engine.
class STagDecoderGPUWorker
{
public:
...
private:
    ...
    std::vector<ConfidenceFilter<Stage2Keypoint, const tcb::span<const GridPriorValue> &, DecoderPredicate>> m_confidenceFilters;
...
private:
    static constexpr size_t m_maxBatchSize = 3;
    ...
    std::array<std::unique_ptr<STagDecoderGPUWorker>, m_maxBatchSize> m_gpuWorkers;
