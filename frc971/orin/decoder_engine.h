...
private:
    void blobFromGpuImageWrappers(const std::vector<GpuImage<uint8_t>>& batchInput) override;
};
...
private:
    nvinfer1::Dims inputDimsFromInputImage(const GpuImage<uint8_t>& batchInput, const nvinfer1::Dims &modelInputDims) override;
    void blobFromGpuImageWrappers(const std::vector<GpuImage<uint8_t>>& batchInput, size_t bufferIdx) override;
